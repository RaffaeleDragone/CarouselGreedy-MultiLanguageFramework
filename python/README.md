...
The behavior of the `carousel_greedy.CarouselGreedy` can be customized through several parameters, which can be overridden when creating the solver instance. Additionally, `alpha`, `beta`, and `feasibility_aware` can be redefined for a minimization run through `minimize()`.
